...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'summary') {
      ...
      const context = await webCrawler.getAllCrawledDataAsContext(userId, maxItems)
      ...
      return NextResponse.json({ error: 'Invalid action. Use summary or context' }, { status: 400 })
    }
