...
export class SearXNGClient {
  ...
  async searchAndCrawl(
    ...
    try {
      ...
      console.log(
        `Found ${urlsToCrawl.length} URLs to crawl for query: "${query}"`
      );
      ...
      for (let i = 0; i < urlsToCrawl.length; i++) {
        ...
        try {
          ...
          // Save to database
          await webCrawler.saveCrawledData(userId, groupId, crawlResult);
      ...
      for (let i = 0; i < urlsToCrawl.length; i++) {
        ...
        try {
          ...
          progressTracker.updateProgress(sessionId, {
            successfulCrawls,
            completedUrls: [
              ...(progressTracker.getProgress(sessionId)?.completedUrls || []),
              url,
            ],
          });
      ...
      const response = await fetch(
        `http://localhost:8080/search?q=test&format=json`,
        {
          headers: {
            Accept: "application/json",
            "User-Agent":
              "Mozilla/5.0 (compatible; MyCrawler/1.0; +http://localhost)",
          },
