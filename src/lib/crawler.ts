...
export class WebCrawler {
  ...
  async saveCrawledData(userId: string, crawlResult: CrawlResult): Promise<void> {
    try {
  ...
  async searchCrawledData(userId: string, query: string): Promise<string> {
    try {
      ...
      return crawledData
        .map(data => `From ${data.title} (${data.url}):\n${data.content.substring(0, 500)}...`)
        .join('\n\n')
    } catch (error) {
  ...
  async getAllCrawledDataAsContext(userId: string, maxItems: number = 10): Promise<string> {
    try {
      const crawledData = await prisma.crawledData.findMany({
        where: {
          userId: userId
